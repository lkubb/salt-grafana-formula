...
We will see a quick setup to configure the ``grafana`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``grafana`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``grafana`` formulas under ``/srv/salt/grafana/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/grafana/parameters/dns:domain/
    ln -s dns:domain /srv/salt/grafana/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/grafana/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/grafana/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``grafana`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``grafana`` formulas under ``/srv/salt/grafana/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/grafana/parameters/roles
...
We create a configuration for the role ``grafana/server`` in ``/srv/salt/grafana/parameters/roles/grafana/server.yaml``:
...
We create another configuration for the role ``grafana/client`` in ``/srv/salt/grafana/parameters/roles/grafana/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles grafana/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles grafana/client
...
.. code-block:: console
    ...
    mv /srv/salt/grafana/parameters/dns:domain/  '/srv/salt/grafana/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-grafana-as-json:
      test.nop:
        - name: {{ grafana | json }}
...
Here is an example based on `template-formula/grafana/config/file.sls`_:
...
This ``sls`` file expose a ``grafana`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ grafana['winner'] }}
...
.. _template-formula/grafana/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/grafana/config/file.sls
